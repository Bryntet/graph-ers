...
impl Function {
    ...
    fn parse(input: &str) -> Result<Self, ParseError> {
        ...
        let function_variables = captures.name("FunctionVariables").ok_or(ParseError::UnableToFind("function variables".to_string()))?.as_str().split(',').map(String::from).collect_vec();

        let ex = captures.name("Expression").ok_or(ParseError::UnableToFind("function expression".to_string()))?.as_str().to_string();
...
mod test {
    ...
    fn unused_variables() {
        ...
        assert_eq!(Function::try_from(test_fn), Err(ParseError::VariableDefinitionAndUseMismatch));
    }
}